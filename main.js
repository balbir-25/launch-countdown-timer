...
function flipAllCards(time) {
  ...
  // Update the days card
  const daysCard = document.querySelector('.days .time1');
  const formattedDays = days.toString().padStart(2, '0'); // Add leading zero if days is a single digit
  daysCard.querySelector('.top').textContent = formattedDays;
  daysCard.querySelector('.bottom').textContent = formattedDays;

  // Update the hours, minutes, and seconds cards
